...
/* The engine frame counter, for anything that needs to know WHICH frame it is
 * acting in. Added round 153 for the touch path: uGUI fires a click even when
 * press and release land in the same frame, LeanTouch does not, so "how many
 * frames was this finger live for" is the one number that separates a working
 * uGUI button from a dead LeanSelectable. */
int diag_current_frame(void) { return g_frame; }
...
static void watchdog_main(void *unused) {
  ...
  for (;;) {
    ...
    /* round 62: bounded periodic sample during the loading phase */
    { static uint64_t last_probe = 0; static int probes = 0;
      if (probes < 12 &&
          (last_probe == 0 || tick_to_ns(now - last_probe) >= 4000000000ull)) {
        last_probe = now; probes++;
        debugPrintf("\n[wd] ---- periodic load-probe #%d ----\n", probes);
        dump_threads(1000 + probes, now);
        last_dump = now;
      }
    }
